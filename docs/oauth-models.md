...
WebSocket commands `models-list`, `models-sync`, `models-migrate`, `models-save` and `models-delete` use the existing authenticated loopback transport. Every write carries the `oauth-models` namespace revision. Conflicts refresh the browser's list and preserve the editor for deliberate retry. `models-list` carries independent `offset` and `manualOffset` values. `models-page` returns up to ten automatic rows and ten manual rows, clamps each offset separately, and includes aggregate counts, connection/source status and the last successful timestamp. Pending rows are excluded from the page. Disconnected accounts return no rows; the browser clears their pages and ignores responses belonging to requests invalidated by sign-out. Account status frames do not carry model directories or tokens.
...
Manual models appear before automatic matches. Editing a saved manual model replaces its row with an inline form; saving or cancelling restores the row. The add-model form remains above the lists.
